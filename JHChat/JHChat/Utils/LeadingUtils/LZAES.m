...
+(NSString *) aes256_encrypt:(NSString *)key Encrypttext:(NSString *)text
{
    ...
    //对数据进行加密
    NSData *result = [LZAES AES256ParmEncryptWithKey:key Encrypttext:data];
...
+(NSString *) aes256_decrypt:(NSString *)key Decrypttext:(NSString *)text
{
    ...
    //对数据进行解密
    NSData* result = [LZAES  AES256ParmDecryptWithKey:key Decrypttext:data];
    if (result && result.length > 0) {
