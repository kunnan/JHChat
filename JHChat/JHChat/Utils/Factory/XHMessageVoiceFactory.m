...
+ (UIImageView *)messageVoiceAnimationImageViewWithBubbleMessageType:(XHBubbleMessageType)type {
    ...
    for (NSInteger i = 0; i < 4; i ++) {
        ...
        UIImage *image = [ImageManager LZGetImageByFileName:[imageSepatorName stringByAppendingFormat:@"VoiceNodePlaying00%ld", (long)i]];
        if (image)
            [images addObject:image];
    }
    
    messageVoiceAniamtionImageView.image = [ImageManager LZGetImageByFileName:[imageSepatorName stringByAppendingString:@"VoiceNodePlaying"]];
