...
@interface TZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate,CLImageEditorDelegate> {
...
- (void)configBottomToolBar {
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 44-LZ_TOOLBAR_HEIGHT, self.view.tz_width, 44)];
    if (STATUS_BAR_BIGGER_THAN_20 && !LZ_IPHONEX_All) {
        _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 44-LZ_TOOLBAR_HEIGHT+20, self.view.tz_width, 44)];
    }
    ...
    if (_tzImagePickerVc.isAllowEditImage) {
        NSString *imageText = [NSBundle tz_localizedStringForKey:@"Edit"];
        CGFloat imageTextWidth = [imageText boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size.width;
        _imageEditButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _imageEditButton.frame = CGRectMake(5, 0, imageTextWidth + 4, 44);
        _imageEditButton.backgroundColor = [UIColor clearColor];
        [_imageEditButton addTarget:self action:@selector(editImage) forControlEvents:UIControlEventTouchUpInside];
        _imageEditButton.titleLabel.font = [UIFont systemFontOfSize:13];
        [_imageEditButton setTitle:imageText forState:UIControlStateNormal];
        [_imageEditButton setTitle:imageText forState:UIControlStateSelected];
        [_imageEditButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_imageEditButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
        _imageEditButton.enabled = _tzImagePickerVc.selectedModels.count <= 1;
    }
    
    
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat x = _imageEditButton ? CGRectGetMaxX(_imageEditButton.frame) : 0;
        ...
        _originalPhotoButton.frame = CGRectMake(x, 0, fullImageWidth + 56, 44);
    ...
    [_toolBar addSubview:_imageEditButton];
...
- (void)configCollectionView {
    ...
    if (STATUS_BAR_BIGGER_THAN_20 && !LZ_IPHONEX_All) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(-10, 0, self.view.tz_width + 20, self.view.tz_height+20) collectionViewLayout:layout];
    }
...
-(void)editImage{
    ...
    if (tzImagePickerVc.selectedModels.count == 0 && tzImagePickerVc.minImagesCount <= 0) {
        ...
        model = [tzImagePickerVc.selectedModels lastObject];
    }
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            for (UIViewController * viewcontroller in viewControllers) {
                if ([viewcontroller isKindOfClass:[TZPhotoPickerController class]]) {
                    editor.delegate = (id)viewcontroller;
                }
            }
            
            NSString *getFileOrgName = [model.asset valueForKey:@"filename"];
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    if ((_tzImagePickerVc.selectedModels.count == 1 && _selectButton.isSelected) || _tzImagePickerVc.selectedModels.count == 0) {
        _imageEditButton.hidden = NO;
    }
    else {
        _imageEditButton.hidden = YES;
       
    }
    if (_isAllowImageEdit) {
        CGFloat X = _imageEditButton.hidden ? 0: CGRectGetMaxX(_imageEditButton.frame);
        NSString *fullImageText = [NSBundle tz_localizedStringForKey:@"Full image"];
        CGFloat fullImageWidth = [fullImageText boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size.width;
        _originalPhotoButton.frame = CGRectMake(X, 0, fullImageWidth + 56, 44);
    }    
