...
@property (nonatomic,weak) id<CLippingDelegate> clippingDelegate;
@property (nonatomic, assign) CGRect clippingRect;
@property (nonatomic, strong) CLRatio *clippingRatio;
@property (nonatomic,assign) CGRect tmpOldRect;
...
@interface CLClippingTool()<CLippingDelegate>
...
//    CGFloat buttonWidth = (swapBtnHidden) ? 0 : 70;
    ...
    _menuScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, _menuContainer.width , _menuContainer.height)];
    _menuScroll.backgroundColor = [UIColor clearColor];
    _menuScroll.showsHorizontalScrollIndicator = NO;
    _menuScroll.clipsToBounds = NO;
    // 修改 编辑菜单
    //_menuContainer.y = self.editor.menuView.frame.origin.y - self.editor.menuView.frame.size.height-5;
...
//    }
//    
    _gridView = [[CLClippingPanel alloc] initWithSuperview:self.editor.view frame:self.editor.imageView.frame];
    ...
    _gridView.clippingDelegate = self;
    _gridView.tmpOldRect = _gridView.clippingRect;
...
#pragma mark - 代理 改变 editBaseView的frame
-(void)clippingGetRect:(CGRect)rect {
    //self.editor.editBaseView.frame = rect;
}
- (void)cleanup
{
...
{    
    completionBlock([self bulidImage], nil, nil);
}

-(UIImage*)bulidImage {
    CGFloat zoomScale = self.editor.imageView.width / self.editor.imageView.image.size.width;
    CGRect rct = _gridView.clippingRect;
    NSLog(@"image的宽和高：%@，%@",NSStringFromCGRect(self.editor.imageView.frame), NSStringFromCGRect(rct) );
   ...
   NSLog(@"image的宽和高：%@，%@",NSStringFromCGRect(self.editor.imageView.frame), NSStringFromCGRect(rct) );
//    [self.editor.editBaseView ]
    UIImage *result = [self.editor.imageView.image crop:rct];
    
    [dic setValue:NSStringFromCGRect(_gridView.clippingRect) forKey:@"newframe"];
    [dic setValue:NSStringFromCGRect(_gridView.tmpOldRect) forKey:@"oldframe"];
    
    for (UIView *classView in [self.editor.editBaseView subviews]) {
        NSLog(@"%@:%@",NSStringFromClass([classView class]),NSStringFromCGRect(classView.frame));
    }
...
- (void)setClippingRect:(CGRect)clippingRect
{
    ...
    if ([self.clippingDelegate respondsToSelector:@selector(clippingGetRect:)]) {
        [self.clippingDelegate clippingGetRect:clippingRect];
    }
