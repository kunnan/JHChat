...
@property (nonatomic, strong) NSMutableArray *tooArray;
@end
@implementation CLImageToolBase
-(NSMutableArray *)tooArray {
    if (_tooArray == nil) {
        _tooArray = [[NSMutableArray alloc] init];
    }
    return _tooArray;
}
- (id)initWithImageEditor:(_CLImageEditorViewController*)editor withToolInfo:(CLImageToolInfo*)info
{
    // 如果 self.toolInfo 已经存在的话 就不再初始化
    
    self = [super init];
    if(self){
        NSLog(@"初始化了编辑工具%@", info.toolName);
        self.editor   = editor;
        self.toolInfo = info;
    }
    return self;
