...
- (void)setup
{
    ...
    //_handlerView = [[UIView alloc] initWithFrame:self.editor.imageView.frame];
    //self.editor.imageView.superview
    [self.editor.view addSubview:self.handlerView];
    [self setHandlerView];
    
    _blurSlider = [self sliderWithValue:0.2 minimumValue:0 maximumValue:1];
    _blurSlider.superview.center = CGPointMake(self.editor.view.width/2, self.editor.menuView.top-self.editor.menuView.frame.size.height-30);
...
//        indicator.center = CGPointMake(self.handlerView.width/2, self.handlerView.height/2);
//        [self.handlerView addSubview:indicator];
...
-(UIImage*)bulidImage {
    UIImage *blurImage = [self.editor.imageView.image gaussBlur:_blurSlider.value]; //_originalImage
    UIImage *image = [self buildResultImage:self.editor.imageView.image withBlurImage:blurImage];
    return image;
}
#pragma mark-
...
- (void)setHandlerView
{
    ...
    [self.handlerView addGestureRecognizer:tapGesture];
    [self.handlerView addGestureRecognizer:panGesture];
    [self.handlerView addGestureRecognizer:pinch];
    [self.handlerView addGestureRecognizer:rot];
}

- (void)setDefaultParams
{
    CGFloat W = 1.5*MIN(self.handlerView.width, self.handlerView.height);
    
    _circleView = [[CLBlurCircle alloc] initWithFrame:CGRectMake(self.handlerView.width/2-W/2, self.handlerView.height/2-W/2, W, W)];
    _circleView.backgroundColor = [UIColor clearColor];
    _circleView.color = [UIColor whiteColor];
    
    CGFloat H = self.handlerView.height;
    CGFloat R = sqrt((self.handlerView.width*self.handlerView.width) + (self.handlerView.height*self.handlerView.height));
    
    _bandView = [[CLBlurBand alloc] initWithFrame:CGRectMake(0, 0, R, H)];
    _bandView.center = CGPointMake(self.handlerView.width/2, self.handlerView.height/2);
...
- (UIImage*)bandBlurImage:(UIImage*)image withBlurImage:(UIImage*)blurImage
{
    ...
    UIGraphicsBeginImageContext(image.size);
    {
        ...
        CGContextTranslateCTM(context, 0, _bandView.offset*image.size.width/self.handlerView.width);
...
- (void)tappedBlurMenu:(UITapGestureRecognizer*)sender
{
    ...
    if(view.tag != _blurType){
        ...
        switch (_blurType) {
            case kCLBlurTypeNormal: // 模糊类型 全模糊
                break;
            case kCLBlurTypeCircle: // 模糊类型 圆圈范围模糊
                [self.handlerView addSubview:_circleView];
                [_circleView setNeedsDisplay];
                break;
            case kCLBlurTypeBand:
                [self.handlerView addSubview:_bandView];
...
- (void)tapHandlerView:(UITapGestureRecognizer*)sender
{
    switch (_blurType) {
        case kCLBlurTypeCircle:
        {
            CGPoint point = [sender locationInView:self.handlerView];
        ...
        {
            CGPoint point = [sender locationInView:self.handlerView];
            point = CGPointMake(point.x-self.handlerView.width/2, point.y-self.handlerView.height/2);
...
- (void)pinchHandlerView:(UIPinchGestureRecognizer*)sender
{
    switch (_blurType) {
        case kCLBlurTypeCircle:
        {
            ...
            CGRect rct;
            rct.size.width  = MAX(MIN(initialFrame.size.width*scale, 3*MAX(self.handlerView.width, self.handlerView.height)), 0.3*MIN(self.handlerView.width, self.handlerView.height));
