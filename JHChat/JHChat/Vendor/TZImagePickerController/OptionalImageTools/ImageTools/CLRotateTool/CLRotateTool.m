...
@implementation CLRotateTool
{
    ...
    //UIScrollView *self.menuScroll; //迁到根了
    CGRect _initialRect;
...
- (void)setup
{
    ...
    _rotateSlider.superview.center = CGPointMake(self.editor.view.width/2, self.editor.menuView.top - 20); // -self.editor.menuView.frame.size.height-30
   
    
    self.menuScroll = [[UIScrollView alloc] initWithFrame:self.editor.menuView.frame];
    self.menuScroll.backgroundColor = self.editor.menuView.backgroundColor;
    self.menuScroll.showsHorizontalScrollIndicator = NO;
    self.menuScroll.alpha = 1;
    // 编辑菜单的位置的修改
    //self.menuScroll.y = self.editor.menuView.frame.origin.y - self.editor.menuView.frame.size.height;
    [self.editor.view addSubview:self.menuScroll];
    [self setMenu];
    
    self.menuScroll.transform = CGAffineTransformMakeTranslation(0, self.editor.view.height-self.menuScroll.top);
    [UIView animateWithDuration:kCLImageToolAnimationDuration
                     animations:^{
                         self.menuScroll.transform = CGAffineTransformIdentity;
                     }
...
{
    CGFloat W = 50;
    CGFloat H = self.menuScroll.height;
    CGFloat x = 0;
    ...
    for(NSDictionary *obj in _menu){
        ...
        [self.menuScroll addSubview:view];
        x += W;
    }
    self.menuScroll.contentSize = CGSizeMake(MAX(x, self.menuScroll.frame.size.width+1), 0);
}
...
- (UISlider*)sliderWithValue:(CGFloat)value minimumValue:(CGFloat)min maximumValue:(CGFloat)max
{
    ...
    slider.value = value;
    // 不起作用
//    container.y =self.editor.menuView.frame.origin.y - self.editor.menuView.frame.size.height-_rotateSlider.height;
