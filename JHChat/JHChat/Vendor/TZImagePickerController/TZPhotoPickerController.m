...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,CLImageEditorDelegate> {
...
//    CGFloat collectionViewHeight = tzImagePickerVc.maxImagesCount > 1 ? self.view.tz_height - 50 - top : self.view.tz_height - top;
    CGFloat collectionViewHeight = self.view.tz_height - 50 - LZ_STATUS_NAV_HEIGHT-LZ_TOOLBAR_HEIGHT ;
    
    _collectionView = [[TZCollectionView alloc] initWithFrame:CGRectMake(0, LZ_STATUS_NAV_HEIGHT, self.view.tz_width, collectionViewHeight) collectionViewLayout:layout];
    if (STATUS_BAR_BIGGER_THAN_20 && !LZ_IPHONEX_All) {
        _collectionView = [[TZCollectionView alloc] initWithFrame:CGRectMake(0, LZ_STATUS_NAV_HEIGHT-20, self.view.tz_width, collectionViewHeight+20) collectionViewLayout:layout];
    }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    [tzImagePickerVc setNavigationBarHidden:NO animated:NO];
...
//    if (tzImagePickerVc.maxImagesCount <= 1) return;
    ...
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 50-LZ_TOOLBAR_HEIGHT, self.view.tz_width, 50)];
...
//    _previewButton.tz_width = tzImagePickerVc.maxImagesCount <= 1 ? 0 : previewWidth + 2;
    _previewButton.tz_width = previewWidth + 2;
    ...
    TZImagePickerController *tzImage = (TZImagePickerController*)self.navigationController;
    if (tzImage.isAllowEditImage) {
        NSString *editText = [NSBundle tz_localizedStringForKey:@"Edit"];
        CGFloat editWidth = [editText boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} context:nil].size.width;
        _editImageButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _editImageButton.frame = CGRectMake(CGRectGetMaxX(_previewButton.frame)+ 5, 3, editWidth + 2, 44);
        _editImageButton.tz_width = previewWidth + 2;
        [_editImageButton addTarget:self action:@selector(editImageButtonClick) forControlEvents:UIControlEventTouchUpInside];
        _editImageButton.titleLabel.font = [UIFont systemFontOfSize:16];
        [_editImageButton setTitle:editText forState:UIControlStateNormal];
        [_editImageButton setTitle:editText forState:UIControlStateDisabled];
        [_editImageButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [_editImageButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
        _editImageButton.enabled = (tzImagePickerVc.selectedModels.count == 1);
    }
    
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        CGRect frame = _editImageButton ? _editImageButton.frame : _previewButton.frame;
        ...
        _originalPhotoButton.frame = CGRectMake(CGRectGetMaxX(frame), self.view.tz_height - 50-LZ_TOOLBAR_HEIGHT, fullImageWidth + 56, 50);
    ...
    [bottomToolBar addSubview:_editImageButton];
...
- (void)editImageButtonClick {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    TZAssetModel *model = [tzImagePickerVc.selectedModels lastObject];
    __weak typeof(self) weakSelf = self;
    [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (isDegraded) return;
        CLImageEditor *editor = [[CLImageEditor alloc] initWithImage:photo];
        
        editor.delegate = self;
        
        [weakSelf pushImageeditViewController:editor asset:model.asset];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo) {
        ...
        TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
//        if (tzImagePickerVc.isAllowEditImage ) {
//            __weak typeof(self) weakSelf = self;
//            [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
//                if (isDegraded) return;
//                CLImageEditor *editor = [[CLImageEditor alloc] initWithImage:photo];
...
//        else {
        ...
        photoPreviewVc.isAllowImageEdit = tzImagePickerVc.isAllowEditImage;
            [self pushPhotoPrevireViewController:photoPreviewVc];
...
#pragma mark - 图片编辑 代理
-(void)imageEditor:(CLImageEditor *)editor didFinishEditingWithImage:(UIImage *)image fileName:(NSString *)filename {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if (tzImagePickerVc.didFinishImageEdit) {
        tzImagePickerVc.didFinishImageEdit(image,filename);
    }
    [editor dismissViewControllerAnimated:YES completion:nil];
    
}
-(void)imageEditSaveToAblum:(CLImageEditor *)editor saveImage:(UIImage *)image {
    
    [self reloadPhotoArray:YES];
    
}
- (void)pushImageeditViewController:(CLImageEditor *)photoPreviewVc asset:(id)assets{
   ;
    NSString *getFileOrgName = [assets valueForKey:@"filename"];
    NSString *exptype = [getFileOrgName pathExtension];
    NSString *fileShowName = [NSString stringWithFormat:@"IMG_%@.%@", [[LZFormat FormatNow2String] substringWithRange:NSMakeRange(10, 4)],exptype];
    photoPreviewVc.fileName = fileShowName;
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        //        UIImage *image = [info objectForKey:UIImagePickerControllerEditedImage];//剪裁后的图片
//        UIImage *image = [info objectForKey:UIImagePickerControllerOriginalImage];//原图
        [[TZImageManager manager] savePhotoWithImage:image completion:^(NSError *error){
            if (!error) {
                [self reloadPhotoArray:NO];
            }
        }];
    }
}

- (void)reloadPhotoArray:(BOOL)isImageEdit {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    [[TZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
        _model = model;
        [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
