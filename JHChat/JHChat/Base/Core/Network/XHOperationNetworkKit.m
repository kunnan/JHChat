...
- (id)initWithRequest:(NSURLRequest *)request
   jsonSuccessHandler:(XHJSONSuccessHandler)jsonSuccessHandler
    ...
    if(self){
        _connection = [[LZURLConnection alloc] initWithRequest:[self request] delegate:self startImmediately:NO];
    }
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            _failureHandler(_connection, _responseData, _response, error);
        });
    }
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if ((success && _successHandler)) {
        dispatch_async(dispatch_get_main_queue(), ^{
            _successHandler(_connection,_responseData, _response);
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            _jsonSuccessHandler(_connection,JSON);
        });
    } else if (!success && _failureHandler) {
        dispatch_async(dispatch_get_main_queue(), ^{
            _failureHandler(_connection,_responseData, _response, nil);
        });
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    if (_progressHandler) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            _progressHandler(_connection,progress, _total);
        });
    }
