...
#pragma mark - 根据 模数、指数 对 密码加密，方式一 (RSA)
+(NSString*)encryptWithPublicKey:(NSString*)modulus exponent:(NSString*)exponent content:(NSString*)content{
    NSData *modulusData=  [NSData dataWithBase64EncodedString:modulus];
    NSData *expoData=  [NSData dataWithBase64EncodedString:exponent];
    
    RSA *publicKey = [DDRSAWrapper publicKeyFormMod:[self stringFromData:modulusData] exp:[self stringFromData:expoData]];
    if (publicKey) {
...
#pragma mark - 根据 模数、指数 对 密码加密，方式二 (SecKey)
+(NSString*)encryptWithPublicKeyRef:(NSString*)modulus exponent:(NSString*)exponent content:(NSString*)content{
    NSData *modulusData=  [NSData dataWithBase64EncodedString:modulus];
    NSData *expoData=  [NSData dataWithBase64EncodedString:exponent];
    
    NSData *publickData = [DDRSAWrapper publicKeyDataWithMod:modulusData exp:expoData];
    SecKeyRef publicKeyRef = [DDRSAWrapper publicSecKeyFromKeyBits:publickData];
    if (publicKeyRef) {
        NSData *plainData = [content dataUsingEncoding:NSUTF8StringEncoding];
